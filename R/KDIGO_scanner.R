...
#' @description All data presented in the KDIGO SCr scanner dummy dataset are entirely fictional and perhaps even nonsensical. They are meant to serve an educational purpose in understanding how the code for AKI diagnosis and staging according to KDIGO AKI guideline criteria, works. Furthermore, while very effort was made to ensure that accurate information was implemented in the code to implement AKI diagnosis and staging according to the KIDIGO AKI guideline serum creatinine criteria, we accept no liability or responsibility to any person or organization as a consequence of any reliance upon the information contained in the code or code use.
...
#'
#' @encoding UTF-8
#' @param data Data frame with one SCr sample per row and the following columns: Admission IDs ('admission' - Integer), admission time ('admission_time' - POSIX), SCr sample timestamps ('time' - POSIX) and SCr sample values ('value' - Numeric) in μmol/L
